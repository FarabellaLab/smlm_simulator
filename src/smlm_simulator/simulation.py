...
'''
def simulateSTORM(moleculeCoords, averageLabelsPerMol, blinkGeomProb,
                  precisionMeanLog, precisionSdLog, detectionRate,
                  falseDetectionRate, fieldLimit, randseed=0, verbose=False):

    # Port of the simulateSTORM function from R to python

    np.random.seed(randseed)

    numDimensions = moleculeCoords.shape[1]
    assert(numDimensions > 1 and numDimensions < 4)
    ...
    if(verbose): print('detectionCoords.shape: ', detectionCoords.shape)
    if(verbose): print('removing undetected blinks')
    detectionIndicesKeep = np.random.choice(np.arange(num_total_blinks), int(detectionRate*num_total_blinks), replace = False)
    detectionCoords = detectionCoords[detectionIndicesKeep]
    blink_to_mol_index = blink_to_mol_index[detectionIndicesKeep]
    if(verbose): print('detectionCoords.shape: ', detectionCoords.shape)

    if(verbose): print('removing detections outside fov')
    if(verbose): print('fieldLimit: ', fieldLimit)
    is_detection_inside = ((detectionCoords > fieldLimit[0])*(detectionCoords < fieldLimit[1])).prod(axis=1).astype('bool')
    detectionCoords = detectionCoords[is_detection_inside]
    blink_to_mol_index = blink_to_mol_index[is_detection_inside]

    if(verbose): print('detectionCoords.shape: ', detectionCoords.shape)

    if(verbose): print('adding false detections')
    numFalseDetections = int(falseDetectionRate*len(detectionCoords))

    if(numFalseDetections > 0):
        falseDetections = np.random.uniform(fieldLimit[0], fieldLimit[1], (numFalseDetections,numDimensions))
        detectionCoords = np.concatenate((detectionCoords, falseDetections), axis=0)
        blink_to_mol_index = np.concatenate((blink_to_mol_index, np.full((numFalseDetections),-1)), axis=0)
    
    if(verbose): print('detectionCoords.shape: ', detectionCoords.shape)

    columns = ['x', 'y'] if numDimensions == 2 else ['x','y','z']
    columns += ['moleculeIndex']
    df = pd.DataFrame(np.concatenate((detectionCoords, np.expand_dims(blink_to_mol_index,1)), axis=1), columns=columns)
    df['moleculeIndex'] = df['moleculeIndex'].astype('int')
    return df
'''
